...
public class PurchaseServiceImplTest {
    ...
    @Test
    @DisplayName("GET findPurchaseListByProductId")
    void findPurchaseListByProductId() {
        // Arrange
        ...
        when(purchaseRepo.findByProductId(1L)).thenReturn(purchaseList);
        when(purchaseMapper.toPurchaseDTOResponse(purchase)).thenReturn(purchaseDTOResponse);
        // Act
        List<PurchaseDTOResponse> resultList = purchaseServiceImpl.findPurchaseListByProductId(1L, 10);
        ...
        // Verify
        InOrder inOrder = Mockito.inOrder(purchaseRepo, purchaseMapper);
        inOrder.verify(purchaseRepo).findByProductId(1L);
        inOrder.verify(purchaseMapper).toPurchaseDTOResponse(purchase);
        inOrder.verifyNoMoreInteractions();
    }
    ...
    @Test
    @DisplayName("GET findPurchaseListByRangePurchaseDate")
    void findPurchaseListByRangePurchaseDate() {
        // Arrange
        Category category = new Category(1L, "Alimentos", "Productos alimenticios", GlobalStatus.ACTIVE);
        Product product = new Product(1L, "Arroz Premium", GlobalUnit.KG, GlobalStatus.ACTIVE, category);
        Supplier supplier = new Supplier(1L, "Fring", "+51920287650", "Fring@email.com", "Lima-Lima", GlobalStatus.ACTIVE);
        ...
        when(purchaseRepo.findByPurchaseDateBetween(start, end)).thenReturn(purchaseList);
        when(purchaseMapper.toPurchaseDTOResponse(purchase)).thenReturn(purchaseDTOResponse);

        // Act
        List<PurchaseDTOResponse> resultList = purchaseServiceImpl.findPurchaseListByRangePurchaseDate(start, end, 10);
        ...
        // Verify
        InOrder inOrder = Mockito.inOrder(purchaseRepo, purchaseMapper);
        inOrder.verify(purchaseRepo).findByPurchaseDateBetween(start, end);
        inOrder.verify(purchaseMapper).toPurchaseDTOResponse(purchase);
        inOrder.verifyNoMoreInteractions();
    }
